...
function Home() {
  ...
  const [statusMessage, setStatusMessage] = useState(
    "Say or type a command such as 'Add milk' or 'Find organic apples'."
  );
  ...
  const buildSmartSuggestions = (currentItems) => {
    ...
    const addSuggestion = (product, reason, allowDuplicate = false) => {
      ...
      const alreadyInList = itemNames.some(
        (name) => productKey.includes(name) || name.includes(productKey)
      );
    ...
    if (lowStockProduct) {
      const breadProduct = availableProducts.find((product) =>
        normalizeText(product.name).includes('bread')
      );
      if (breadProduct) {
    ...
    const previousProduct = [...shoppingHistory]
      .reverse()
      .find((entry) =>
        availableProducts.some((product) => normalizeText(product.name) === normalizeText(entry))
      );
    if (previousProduct) {
      const productMatch = availableProducts.find(
        (product) => normalizeText(product.name) === normalizeText(previousProduct)
      );
      if (productMatch) {
        const relatedProduct = availableProducts.find((product) => {
          return (
            product.category === productMatch.category &&
            normalizeText(product.name) !== normalizeText(productMatch.name)
          );
        });
        if (relatedProduct) {
          addSuggestion(
            relatedProduct,
            `Based on your previous picks, ${relatedProduct.name} fits your usual routine.`
          );
        }
      }
    }
    ...
    if (itemNames.some((name) => name.includes('milk'))) {
      const substitute = availableProducts.find((product) =>
        normalizeText(product.name).includes('almond milk')
      );
      if (substitute) {
    ...
    if (itemNames.some((name) => name.includes('bread'))) {
      const pairing = availableProducts.find((product) =>
        normalizeText(product.name).includes('butter')
      );
      if (pairing) {
    ...
    seasonalProducts.forEach((productName) => {
      const product = availableProducts.find((entry) =>
        normalizeText(entry.name).includes(productName)
      );
      if (product) {
  ...
  const inferCategory = (name) => {
    ...
    if (
      /apple|banana|orange|grape|tomato|lettuce|carrot|potato|onion|fruit|vegetable/i.test(
        normalized
      )
    )
      return 'Produce';
    ...
    return (
      availableProducts.find((product) => {
        ...
        const searchText = [
          ...
          .join(' ');
        return (
          productName === normalizedInput || searchText.includes(normalizedInput) || aliasMatch
        );
      }) || null
    );
  };
  ...
  const parseItemDetails = (rawText) => {
    ...
    const quantity = quantityMatch
      ...
      : quantityWordMatch
      ? quantityMap[quantityWordMatch[1].toLowerCase()]
      : 1;

    const brandMatch = text.match(
      /\b(organic|vegan|gluten-free|low-sugar|premium|store-brand|generic)\b/i
    );
    const sizeMatch = text.match(
      /\b(\d+\s?(ml|l|kg|g|oz|lb|cm|inch|inches)|small|medium|large|family|jumbo|mini|regular)\b/i
    );
    const priceMatch = text.match(
      /under\s*\$?(\d+(?:\.\d{1,2})?)|\$?(\d+(?:\.\d{1,2})?)\s*(or|and)\s*\$?(\d+(?:\.\d{1,2})?)/i
    );
    ...
    const cleaned = text
      .replace(
        /^(add|buy|purchase|need|i need|i want|i want to buy|i need to buy|put|include|agrega|añade|comprar|necesito|quiero|quiero comprar|pon|incluye)\s+/i,
        ''
      )
  ...
  const processCommand = async (rawText) => {
    ...
    try {
      const removeMatch = normalized.match(
        /^(remove|delete|eliminate|take out|quit|quitar|borrar|sacar|elimínate|eliminar)\b/i
      );
      if (removeMatch) {
        const itemName = text
          .replace(removeMatch[0], '')
          .replace(/^(from|my|the|list|on|in)/i, '')
          .trim();
      ...
      const searchMatch = normalized.match(
        /^(find|search|look for|buscar|encuentra|ver|revisar)\b/i
      );
      if (searchMatch) {
        const query = text
          .replace(searchMatch[0], '')
          .replace(/^(for|me|the|my|list)/i, '')
          .trim();
      ...
      const modifyMatch = normalized.match(
        /^(change|update|replace|swap|modify|cambiar|actualizar|reemplazar|cambia)\b/i
      );
      if (modifyMatch) {
        ...
        const match = items.find((item) =>
          item.name.toLowerCase().includes(originalName.toLowerCase())
        );
        if (!match || !newName) {
        ...
        const response = await api.put(`/items/${match._id}`, {
          name: newName,
          category: inferCategory(newName),
        });
        setItems((currentItems) =>
          currentItems.map((item) =>
            item._id === match._id ? response.data.data || response.data : item
          )
        );
      ...
      if (!catalogMatch) {
        const fallbackSuggestion = availableProducts.find(
          (product) =>
            normalizeText(product.name).includes(normalizeText(name)) ||
            normalizeText(name).includes(normalizeText(product.name))
        );
        if (fallbackSuggestion) {
          setStatusMessage(
            `"${name}" was not found exactly, but ${fallbackSuggestion.name} looks like a close match.`
          );
          setCommand('');
          setTranscript(text);
          setIsLoading(false);
      ...
      try {
        ...
        setShoppingHistory((currentHistory) => {
          ...
          if (
            !nextHistory.some((entry) => normalizeText(entry) === normalizeText(catalogMatch.name))
          ) {
        ...
        setStatusMessage(
          `Added ${quantity} ${payload.unit} ${catalogMatch.name} from the product catalog.`
        );
      } catch (error) {
        console.error(error);
        ...
        fetchSuggestions(nextItems);
        setStatusMessage(
          `Added ${quantity} ${payload.unit} ${catalogMatch.name} to the shopping list.`
        );
      }
      setCommand('');
      setTranscript(text);
  ...
  const handleVoiceSearch = () => {
    ...
    recognition.onresult = (event) => {
      const finalText = Array.from(event.results)
        .map((result) => result[0].transcript)
        .join(' ');
      const cleanedQuery = finalText
        .replace(/^(find|search|look for|buscar|encuentra|ver|revisar)\s+/i, '')
        .trim();
      setTranscript(finalText);
      setSearchQuery(cleanedQuery);
      setStatusMessage(
        cleanedQuery ? `Showing matches for "${cleanedQuery}".` : 'Showing your current list.'
      );
      setIsVoiceSearchListening(false);
    };
    recognition.onerror = (event) => {
    ...
    try {
      ...
      setItems((currentItems) =>
        currentItems.map((currentItem) =>
          currentItem._id === item._id ? response.data.data || response.data : currentItem
        )
      );
    } catch (error) {
      console.error(error);
  ...
  const filteredItems = useMemo(() => {
    ...
    const priceMatch = query.match(
      /under\s*\$?(\d+(?:\.\d{1,2})?)|less\s+than\s*\$?(\d+(?:\.\d{1,2})?)|between\s*\$?(\d+(?:\.\d{1,2})?)\s*(and|to)\s*\$?(\d+(?:\.\d{1,2})?)/i
    );
    ...
    return items.filter((item) => {
      ...
      const matchesPrice = maxPrice
        ...
        : minPrice && maxPrice2
        ? Number(item.price || 0) >= minPrice && Number(item.price || 0) <= maxPrice2
        : true;
      const matchesBrand = query.includes('organic')
        ...
        <div className="hero" style={styles.heroSection}>
          <div>
            ...
            <p style={styles.subtitle}>
              Use your voice or type a command, then let the app categorize, suggest, and manage the
              list for you.
            </p>
          </div>
            ...
            <SuggestionPanel
              ...
              onAdd={(name) =>
                handleAddFromCatalog(
                  availableProducts.find((product) => product.name === name) || { name }
                )
              }
            />
